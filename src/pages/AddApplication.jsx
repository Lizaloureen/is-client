...
const AddApplication = () => {
  ...
  return (
    ...
    <div >
        <form onSubmit={handleSubmit} style={{backgroundColor: 'white', padding: '20px', borderRadius: '10px', marginTop:'50px'}}>
        <h3 className='flex-center'>Add application</h3>
            ...
            <select id = "firearm-type" name='type_of_firearm' required value={formData.type_of_firearm}  className='input' onChange={handleChange}>
                ...
                {fa_types.map((fa, index) => (
                  <option key={index} value={fa}>{fa}</option>
                ))}
            </select>
            
            <label htmlFor="reason">Reason for Firearm</label>
            <textarea onChange={handleChange} name='reason' required value={formData.reason}>Reason</textarea>
            
            <label htmlFor = "Next Of Kin">Next of Kin Name</label>
            <input onChange={handleChange} name='next_of_kin_name' required value={formData.next_of_kin_name} type = "text" id = "next_of_kin_name" placeholder="Next Of Kin name"/>

            <label htmlFor = "address">Address</label>
            <textarea  onChange={handleChange} name='address' required value={formData.address}>Enter your Address</textarea>
            
            <label htmlFor = "region">Region *</label>
            <select id = "region" name='region' required value={formData.region}  className='input' onChange={handleChange}>
                <option value = "">--Select---</option>
                {counties.map((county, i) => (
                  <option key={i} value={county.name}>{county.name}</option>
                ))}
            </select>

          
              <label htmlFor="goodConduct">Attach Good Conduct *</label>
              <input id="goodConduct"
                name='goodConduct'
